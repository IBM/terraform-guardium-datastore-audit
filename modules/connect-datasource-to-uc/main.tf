locals {
  ...
  # Use configurable log directory with sensible default
  log_directory = var.log_directory != "" ? var.log_directory : "/var/log/guardium"
}
...
resource "terraform_data" "copy_csv" {
  ...
  provisioner "remote-exec" {
    inline = [
      "rm -f ${self.input.log_directory}",  # Remove if it's a file
      "mkdir -p ${self.input.log_directory}",  # Create directory
